...
function Register() {
    ...
    <div className='relative flex flex-col justify-center items-center'>
        ...
        <div className='rounded-xl bg-white px-20 pt-7 pb-10 flex flex-col justify-between gap-9 items-center w-full max-w-96 md:max-w-2xl'>
            ...
            {getsuccess === true && <div className='bg-green-500 h-16 w-96 rounded-2xl flex justify-center items-center top-72 text-black'>{mess}</div>}
            {getsuccess === false && <div className='bg-red-500 h-16 w-96 rounded-2xl flex justify-center items-center top-72 text-white'>{mess}</div>}
            <form className='flex flex-col gap-6' onSubmit={handleSubmit(onSubmit)}>
                ...
                <div className={'flex justify-between items-center border-gray-400 border w-full rounded pr-5 overflow-hidden'
                    ...
                    {isShow ? <input className={'outline-none text-gray-700 w-full box-border py-5 pl-6 ' + 
                ...
                <input className={'outline-none text-gray-700 w-full box-border py-5 pl-6 ' + 
                    (errors.password?.message ? ' placeholder:text-red-400':'')} 
                    type="password" id='password' {...register('password', {required:true})} placeholder='Enter your password' />}
                
                <div onClick={(()=>setShow(!isShow))}>
                    {!isShow ? <FaEye className={'h-7 w-7 text-gray-400'
                    + (errors.password?.message ? ' text-red-600' : '')
                }/> : <FaEyeSlash className={'h-7 w-7 text-gray-400'
                    + (errors.password?.message ? ' text-red-600' : '')}/>}
